...
export default async function EmployeeDashboard({ searchParams }: {
  ...
  // ДОБАВЛЕНО: registration: true, чтобы сервер получил логин из базы
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        {section === 'overview' && (<>
          <div className="backdrop-blur-md bg-white/80 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 shadow-xl border border-white/20">
            {/* ИСПРАВЛЕНО: Теперь берет логин из базы (или имя, если логина почему-то нет) */}
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 truncate">Добрый день, {user.registration?.login || user.fullName}!</h1>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0">
            {[
              ...
              <Link key={t.key} href={`/dashboard?section=documents&tab=${t.key}`}
                className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-all whitespace-nowrap shrink-0 ${tab === t.key ? `bg-gradient-to-r ${t.gradient} text-white shadow-lg ${t.shadow}` : 'backdrop-blur-md bg-white/80 text-gray-700 hover:bg-white/90 border border-white/20 shadow-sm'
                  }`}
              >
                <span>{t.label}</span>
